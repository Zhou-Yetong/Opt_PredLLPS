...
def run(hhb, input, tg, tg_hhm, tmp_folder, db):
    ...
    parameters:
        ...
        :param tmp_folder: tmp folder saving the .fasta files containing a singe sequence
        that is split from the input files.
        tmp文件夹保存. fasta文件，其中包含从输入文件中分割出来的单一序列。
...
def main(args):
    ...
    if not os.path.exists(tg + pt):
        os.makedirs(tg + pt)
    if not os.path.exists(tg_hhm+ pt):
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', type=str, default='utils//hhblits/uniclust30_2018_08/uniclust30_2018_08',help='Database that is searched against')
    parser.add_argument('-i', type=str, default='POS/POS.fasta', help='Input files in fasta format')
    parser.add_argument('-oa3m', type=str, default='POS/a3m/', help='Output folder saving o3m files')
    parser.add_argument('-ohhm', type=str, default='POS/hhm/', help='Output folder saving hhm files')
